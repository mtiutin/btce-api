...
/**
 * This is the only class required to connect to the BTC-E bitcoin exchange.
 * This class allows you to execute API calls on your account to obtain
 * information and create and cancel trades.
 * <p>
 * The BTC-E API is documented at the following location <a
 * href="https://btc-e.com/api/documentation"
 * >https://btc-e.com/api/documentation</a>.
 * <p>
 * 
 * <pre>
 * BTCE btce = new BTCE();
...
public class BTCE {
	...
	public BTCE() {
		...
		gson_builder.registerTypeAdapter(TransactionHistoryReturn.class,
				new TransactionHistoryReturnDeserializer());
		gson_builder.registerTypeAdapter(TradeHistoryReturn.class,
				new TradeHistoryReturnDeserializer());
		gson_builder.registerTypeAdapter(OrderListReturn.class,
				new OrderListReturnDeserializer());
	 ...
	 * Returns the transaction history for the account.
	 * <p>
	 * All arguments can be either a 0, a null or an empty string "" which will
	 * result in the argument being omitted from the API call.
	 * 
	 * @param from
	 *            the number of the starting transaction to include in the
	 *            returned results.
	 * @param count
	 *            the number of transactions to return.
	 * @param from_id
	 *            the starting id of the transaction to include in the return
	 *            results.
	 * @param end_id
	 *            the last id of the transaction to include in the return
	 *            results.
	 * @param order
	 *            the order to sort the returned results. Can be "ASC" or
	 *            "DESC". The default value is "DESC" if this value is null or
	 *            an empty string.
	 * @param since
	 *            starting time frame to include results in unix time stamp
	 *            format.
	 * @param end
	 *            ending time frame to include results in unix time stamp
	 *            format.
	 * @return the transaction history.
	 */
	public TransactionHistory getTransactionHistory(int from, int count,
			int from_id, int end_id, String order, long since, long end)
			throws BTCEException {
		Map<String, String> args = new HashMap<String, String>();
		if (from > 0)
			args.put("from", Integer.toString(from));
		if (count > 0)
			args.put("count", Integer.toString(count));
		if (from_id > 0)
			args.put("from_id", Integer.toString(from_id));
		if (end_id > 0)
			args.put("end_id", Integer.toString(end_id));
		if (order != null && order.length() > 0)
			args.put("order", order);
		if (since > 0)
			args.put("since", Long.toString(since));
		if (end > 0)
			args.put("end", Long.toString(end));
		return gson.fromJson(authrequest("TransHistory", args),
	 ...
	 *            format.
	 * @param pair
	 *            the pair to include in the trade history.
	 * @return the trade history.
	 */
	public TradeHistory getTradeHistory(int from, int count, int from_id,
			int end_id, String order, long since, long end, String pair)
			throws BTCEException {
		Map<String, String> args = new HashMap<String, String>();
		if (from > 0)
			args.put("from", Integer.toString(from));
		...
		if (pair != null && pair.length() > 0)
			args.put("pair", pair);
		return gson.fromJson(authrequest("TradeHistory", args),
				TradeHistory.class);
	}

	/**
	 * @deprecated Returns the order list for the account.
	 * 
	 ...
	 * @deprecated Returns the order list for the account.
	 *             <p>
	 *             All arguments can be either a 0, a null or an empty string ""
	 *             which will result in the argument being omitted from the API
	 *             call.
	 * 
	 * @param from
	 *            the number of the starting transaction to include in the
	 ...
	 *            format.
	 * @param pair
	 *            the pair to include in the order list.
	 * @param active
	 *            include only active orders in the order list.
	 */
	@Deprecated
	public OrderList getOrderList(int from, int count, int from_id, int end_id,
			String order, long since, long end, String pair, int active)
			throws BTCEException {
		Map<String, String> args = new HashMap<String, String>();
		if (from > 0)
			args.put("from", Integer.toString(from));
		...
		if (pair != null && pair.length() > 0)
			args.put("pair", pair);
		if (active > 0)
			args.put("active", Long.toString(active));
	...
	public OrderList getActiveOrders(String pair) throws BTCEException {
		...
		if (pair != null && pair.length() > 0)
			args.put("pair", pair);
		return gson
				.fromJson(authrequest("ActiveOrders", args), OrderList.class);
	}
	 ...
	 * 
	 * @param pair
	 *            the currency pair to trade. This is specified as
	 *            <curr1>_<curr2>, example: "BTC_USD"
	 * @param type
	 *            the type of transaction. Can be "buy" or "sell".
	 * @param rate
	 *            the rate to pay for the transaction in <curr2>.
	 * @param amount
	 *            the quantity of <curr1> to buy.
	 * @return the trade results.
	 */
	public Trade trade(String pair, String type, double rate, double amount)
			throws BTCEException {
		Map<String, String> args = new HashMap<String, String>();
		args.put("pair", pair);
		args.put("type", type);
	...
	public CancelOrder cancelOrder(int order_id) throws BTCEException {
		...
		return gson.fromJson(authrequest("CancelOrder", args),
				CancelOrder.class);
	}
	...
	public void setAuthKeys(String key, String secret) throws BTCEException {
		...
		try {
			...
			throw new BTCEException("HMAC-SHA512 doesn't seem to be installed",
					uee);
		}
	 ...
	 * <p>
	 * Since this call doesn't use authorization, the request limit is typically
	 * set higher (10-15 seconds) to avoid abuse.
	 * 
	 * @param pair
	...
	public Ticker getTicker(String pair) throws BTCEException {
		TickerWrapper tw = gson.fromJson(request(TICKER_TRADES_URL + pair
				+ "/ticker"), TickerWrapper.class);
		return tw.ticker;
	}
	...
	public TradesDetail[] getTrades(String pair) throws BTCEException {
		return gson.fromJson(request(TICKER_TRADES_URL + pair + "/trades"),
				TradesDetail[].class);
	}
	 ...
	 * @throws BTCEException
	 */
	public Depth getDepth(String pair) throws BTCEException {

		return gson.fromJson(request(TICKER_TRADES_URL + pair + "/depth"),
				Depth.class);
	}
	...
	private final String authrequest(String method, Map<String, String> args)
			throws BTCEException {
		if (!initialized)
			throw new BTCEException("BTCE not initialized.");
	...
	/**
	 * Displays the amounts of various currencies associated with an account or
	 * an order.
	 */
	public class Funds {
		public double usd;
		...
		public String toString() {
			return "[usd=" + usd + ", btc=" + btc + ", ltc=" + ltc + ", nmc="
					...
					+ ", trc=" + trc + ", ppc=" + ppc + ", ftc=" + ftc
					+ ", cnc=" + cnc + "]";
		}
	}

	/**
	 * returned by the {@link #getInfo() getInfo} method. Note that due to a
	 * java name collision the JSON field return is named info.
	 */
	public static class Info extends Results {
		...
		@Override
		public String toString() {
			return "Info [success=" + success + ", error=" + error
					+ ", return=" + info + "]";
		}
	...
	public class InfoReturn {
		...
		public String toString() {
			return "[funds=" + funds + ", rights=" + rights
					...
					+ ", open_orders=" + open_orders + ", server_time="
					+ server_time + "]";
		}
	}
	...
	/**
	 * returned by the {@link #getTransactionHistory() getTransactionHistory}
	 * method. Note that due to a java name collision the JSON field return is
	 * named info.
	 */
	public static class TransactionHistory extends Results {
		...
		@Override
		public String toString() {
			return "TransactionHistory [success=" + success + ", error="
					+ error + ", return=" + info + "]";
		}
	}
	...
	public class TransactionHistoryOrderDetails {
		...
		public String toString() {
			return "[type=" + type + ", amount=" + amount + ", currency="
					+ currency + ", desc=" + desc + ", status=" + status
					+ ", timestamp=" + timestamp + "]";
		}
	}
	 ...
	 * 
	 */
	private class TransactionHistoryReturnDeserializer implements
			JsonDeserializer<TransactionHistoryReturn> {
		public TransactionHistoryReturn deserialize(JsonElement json,
			...
			if (json.isJsonObject()) {
				...
				while (iter.hasNext()) {
					...
					transaction.trans_details = context.deserialize(
							jsonOrder.getValue(),
							TransactionHistoryOrderDetails.class);
					transactions.add(transaction);
				}
			}
			thr.transactions = transactions
					.toArray(new TransactionHistoryOrder[0]);
			return thr;
		}
	}

	/**
	 * returned by the {@link #getTradeHistory() getTradeHistory} method. Note
	 * that due to a java name collision the JSON field return is named info.
	 */
	public static class TradeHistory extends Results {
		...
		@Override
		public String toString() {
			return "TradeHistory [success=" + success + ", error=" + error
					+ ", return=" + info + "]";
		}
	...
	public class TradeHistoryOrderDetails {
		...
		@Override
		public String toString() {
			return "[pair=" + pair + ", type=" + type + ", amount=" + amount
					+ ", rate=" + rate + ", order_id=" + order_id
					+ ", is_your_order=" + is_your_order + ", timestamp="
					+ timestamp + "]";
		}
	}

	private class TradeHistoryReturnDeserializer implements
			JsonDeserializer<TradeHistoryReturn> {
		public TradeHistoryReturn deserialize(JsonElement json, Type typeOfT,
			...
			if (json.isJsonObject()) {
				...
				while (iter.hasNext()) {
					...
					trade.trade_details = context.deserialize(
							jsonOrder.getValue(),
							TradeHistoryOrderDetails.class);
					trades.add(trade);
				}
			}
	...
	/**
	 * returned by the {@link #getOrderList() getOrderList} method. Note that
	 * due to a java name collision the JSON field return is named info.
	 */
	public static class OrderList extends Results {
		...
		@Override
		public String toString() {
			return "OrderList [success=" + success + ", error=" + error
					+ ", return=" + info + "]";
		}
	}
	...
	public class OrderListOrderDetails {
		...
		public String toString() {
			return "[pair=" + pair + ", type=" + type + ", amount=" + amount
					+ ", rate=" + rate + ", status=" + status + ", timestamp="
					+ timestamp + "]";
		}
	}

	private class OrderListReturnDeserializer implements
			JsonDeserializer<OrderListReturn> {
		public OrderListReturn deserialize(JsonElement json, Type typeOfT,
				JsonDeserializationContext context) throws JsonParseException {
			...
			if (json.isJsonObject()) {
				...
				while (iter.hasNext()) {
					...
					order.order_details = context.deserialize(
							jsonOrder.getValue(), OrderListOrderDetails.class);
					orders.add(order);
				}
			}
	...
	/**
	 * returned by the {@link #trade(String,String,double,double) trade} method.
	 * Note that due to a java name collision the JSON field return is named
	 * info.
	 */
	public static class Trade extends Results {
		...
		@Override
		public String toString() {
			return "Trade [success=" + success + ", error=" + error
					+ ", return=" + info + "]";
		}
	}
	...
	public class TradeReturn {
		...
		@Override
		public String toString() {
			return "[received=" + received + ", remains=" + remains
					+ ", order_id=" + order_id + ", funds=" + funds + "]";
		}
	}

	/**
	 * returned by the {@link #cancelOrder(int) cancelOrder} method. Note that
	 * due to a java name collision the JSON field return is named info.
	 */
	public static class CancelOrder extends Results {
		...
		@Override
		public String toString() {
			return "CancelOrder [success=" + success + ", error=" + error
					+ ", return=" + info + "]";
		}
	}
	...
	public class CancelOrderReturn {
		...
		@Override
		public String toString() {
			return "[order_id=" + order_id + ", funds=" + funds + "]";
		}
	}
	...
	/**
	 * An exception class specifically for the BTCE API. The goal here is to
